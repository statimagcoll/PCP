...
paramStatMap = lmPBJ(dat$files, form=lmfull, formred=lmred, mask=maskfile, template = templatefile, data=dat, transform = 't', HC3 = TRUE, robust = FALSE)
...
robustStatMap <- lmPBJ(dat$files, form=lmfull, formred=lmred, mask=maskfile, template = templatefile, data=dat, transform = 't', HC3 = TRUE, robust = TRUE)
# Visualize the chi square image
# statmapFile = paste0(tempfile(), '.nii.gz') # generate a random file name
...
## Split sample CAT plot

Reviewers asked to add an analysis splitting the sample and comparing the results across the two sample. I'm doing that here using a concordance at the top plot.
...
#CATplot(rank(robustStatMap1$stat), rank(robustStatMap2$stat), maxrank = 1000)
#CATplot(rank(paramStatMap1$stat), rank(paramStatMap2$stat), maxrank = 200)
#plot(paramStatMap1$stat, paramStatMap2$stat)
```
