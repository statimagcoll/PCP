...
simConfig = list(
  ...
  dat = dat,
  mask = maskfile,
  rdsOutput = '~/Dropbox (VUMC)/pbj/pbj_ftest/transform_images.rds')
)
...
simFunc = function(lmfull, lmred, mask, data){
  list('t'=lmPBJ(data$images, form=lmfull, formred=lmred, mask=mask, data=data, transform = 't')$stat,
...
# simFunc(lmfull= simConfig$form,
#   lmred = simConfig$formred,
#   mask = simConfig$mask,
#   data=simConfig$dat[1:50,])

simdirs$simres = runSim(simdirs$simdir,
...
dir.create(basename(simConfig$rdsOutput), showWarnings = FALSE, recursive = TRUE)

# clean up files
saveRDS(simdirs, file=simConfig$rdsOutput)
