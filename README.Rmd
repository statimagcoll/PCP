...
maskfile = '~/Dropbox/pbj/data/abide/neuroimaging/cpac/n1035_mask.nii.gz'
...
simConfig = list(
  ...
  dat = dat,
  mask = maskfile,
  output = '~/Dropbox (VUMC)/pbj/pbj_ftest/transform_images.rdata'
)
...
simFunc = function(lmfull, lmred, mask, data){
  simplify2array(list('t'=lmPBJ(data$images, form=lmfull, formred=lmred, mask=mask, data=data, transform = 't')$stat,
  'edgeworth'=lmPBJ(data$images, form=lmfull, formred=lmred, mask=mask, data=data, transform = 'edgeworth')$stat,
  'none'=lmPBJ(data$images, form=lmfull, formred=lmred, mask=mask, data=data, transform = 'none')$stat
  ) )
}
...
results = runSim(simdirs$simdir[1], method='synthetic',
...
# clean up files
save.image(file="~/Dropbox/pbj/pbj_ftest/transform_images.rdata")
Sys.sleep(5*60)
...
by(simdirs, simdirs$n, function(x) apply(rowMeans(simplify2array(x$results[!is.na(x$results)]), dims = 2), 2, quantile))
by(simdirs, simdirs$n, function(x) apply(apply(simplify2array(x$results[!is.na(x$results)]), 1:2, var ), 2, quantile, na.rm=TRUE) )
by(simdirs, simdirs$n, function(x) apply(apply(simplify2array(x$results[!is.na(x$results)]), 1:2, function(y) var(y)/mean(y) ), 2, quantile, na.rm=TRUE) )
by(simdirs, simdirs$n, function(x) apply(apply(simplify2array(x$results[!is.na(x$results)]), 1:2, function(y) sd(y)/sqrt(length(y)) ), 2, mean) )
```
