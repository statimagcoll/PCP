...
system(paste0('sed s/###PASSWORD###/', computeConfig$password, '/ ', redisConfTemplate, ' > ', redisConf))
system(paste0('redis-server ', redisConf), wait=FALSE)
registerDoRedis(computeConfig$queuename, nodelay=FALSE, password=computeConfig$password)
```
...
startLocalWorkers(n=computeConfig$ncores, queue=computeConfig$queuename, host=computeConfig$host, password = computeConfig$password,
...
dbxcli('get', dbimages, dat$images, ncores=computeConfig$ncores)
...
simConfig$dat = simConfig$dat[apply(!is.na(simConfig$dat[ ,c(all.vars(as.formula(simConfig$formres)), simConfig$varvar)]), 1, all), ]
if(class(simConfig$formres)=='formula' | is.character(simConfig$formres)){
  simConfig$dat$rfiles = file.path('~/data/res_images', basename(simConfig$dat$images))
  pbj::residualizeImages(files=simConfig$dat$images, dat=simConfig$dat, mask=maskfile, form=simConfig$formres,
...
NIsim::simSetup(simConfig$dat$rfiles, data=simConfig$dat, outdir=simConfig$outdir)
